...
public partial class VoteInPerson
{
    ...
    {
        UserId = await _localStorage.GetItemAsync<int>("currentUserId");

        var votingEligibility = await ApiRequestService.GetInPersonVotingEligibility(UserId);
        if (votingEligibility.Error is null)
            VotingEligibility = votingEligibility.Data;
        else
            Errors.Add(votingEligibility.Error);

        var votedInElections = await ApiRequestService.GetCustomerVotedInElections(UserId);
    ...
    private async Task GenerateQRCode()
    {
        ...
        var qrCodeText = $"UserId: {UserId}, VotedIn: {VotedInElectionIds}, RandomNumber: {randomNumber}, ExpiryDate: {expireyDate}";
